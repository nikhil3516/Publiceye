...
import { Home, Bell, Plus, FileText, User, Navigation, ArrowLeft } from 'lucide-react';
import { Button } from '@/components/ui/button';

export function NearbyComplaintsMockup() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100vh-4rem)] p-4 bg-slate-50 dark:bg-slate-900">
      ...
      <div className="mb-4 text-center">
        <h3 className="text-lg font-black text-slate-800 dark:text-white">Nearby Complaints Mockup</h3>
      ...
      <div className="w-[375px] h-[720px] rounded-[3rem] border-4 border-dashed border-blue-500 bg-white shadow-2xl flex flex-col overflow-hidden relative select-none">
        ...
        {/* Mockup Body */}
        <div className="flex-1 bg-[#F0F0F5] flex flex-col items-center p-6 relative">
          
          {/* Centered Empty State Content (with generous space below) */}
          <div className="flex flex-col items-center text-center mt-28">
            <div className="w-[75px] h-[75px] rounded-full bg-[#B2DFDB] flex items-center justify-center shadow-inner mb-5 transition-transform hover:scale-105 duration-300">
              {/* Navigation / location arrow pointing top-right in teal, outline, ~30px */}
              <Navigation className="w-[30px] h-[30px] text-[#009688] rotate-45" strokeWidth={2} />
            </div>
            <p className="text-sm text-slate-500 font-normal tracking-tight">No Records Found - Yet.</p>
          </div>
...
export default NearbyComplaintsMockup;
